...
// Clean encoding issues (fixes ΓÇô -> –)
...
export const medications: Medication[] = cogsData.map((item, index) => {
  ...
  const cleanTreatment = cleanText(item.treatment);
  ...
  const name = `${cleanTreatment} - ${item.rxDetails} (${item.paymentTerm})`;
  ...
  // Extract category (everything before the dash)
  const categoryMatch = cleanTreatment.match(/^([^–]+)/);
  const category = categoryMatch ? categoryMatch[1].trim() : cleanTreatment;
